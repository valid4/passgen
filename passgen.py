...
class Gen(object):
	...
	def CheckBase(self):
 		sqlite_connection = sqlite3.connect('password.db')
        cursor = sqlite_connection.cursor()
        print("Подключен к SQLite")

        sqlite_select_query = """SELECT * from password"""
	...
	def Delite(self):
		...
		Login = input('Удалить запись\n')
		cur.execute("DELETE FROM password WHERE Login;")
		conn.commit()'''